...
function Accordion({ icon, title, children, defaultOpen = false, badge = null, variant = 'default' }) {
  ...
  const titleColor = variant === 'warning' ? 'text-yellow' : 'text-text';

  return (
    <div className={`glass border ${variantClass} rounded-2xl overflow-hidden mb-3`}>
      <button
        type="button"
      ...
      >
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <h4 className={`font-brico font-semibold text-sm ${titleColor}`}>
            {title}
          </h4>
          {badge}
        </div>
        <span className={`text-xl shrink-0 transition-transform duration-300 ${open ? 'rotate-45' : ''} ${titleColor}`} aria-hidden="true">
          +
        </span>
      </button>
      {open && (
        <div className="px-5 pb-5 pt-1 animate-fadeInUp">
          {children}
        </div>
      )}
    </div>
  ...
  return (
    <div className="w-full animate-fadeInUp">
      ...
      <button
        ...
        className="w-full mb-4 py-4 rounded-2xl bg-gradient-to-r from-[#25D366] to-[#128C7E] text-white font-brico font-bold text-base shadow-lg hover:shadow-xl hover:scale-[1.01] transition-all duration-200 flex items-center justify-center gap-3 group"
      ...
      >
        ...
        <span className="flex-1 text-left">
          <span className="block text-sm opacity-90 font-normal">Reservar pelo WhatsApp</span>
      ...
      {/* Accordion: Contexto */}
      <Accordion icon="🧭" title="Contexto desta data no mês">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-[10px] text-muted uppercase tracking-wider mb-1">Classificação API</p>
      ...
      <Accordion
        ...
        variant="warning"
        badge={<span className="text-[10px] px-2 py-0.5 rounded-full bg-yellow/20 text-yellow font-semibold">Leia</span>}
      >
